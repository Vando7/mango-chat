...
- **`src/api/client.js`** — API client. `fetchModels` tries LM Studio's `/api/v0/models` first (rich metadata: `type`, `publisher`, `arch`, `quantization`, `state`, `maxContext`, `loadedContext`, `capabilities`); falls back to OpenAI `/v1/models` returning `{id}` objects only. `chat` async generator yields `{text, reasoning, toolCalls, finishReason}` per chunk and accepts a 4th `tools` arg (OpenAI function-tool array) — when present, `tool_choice: 'auto'` is sent. Streamed `delta.tool_calls[]` are accumulated index-keyed (each chunk concatenates `function.arguments` partial JSON onto the previous). `setApiBase` mutates the shared base URL. Always returns *objects*, never bare strings — callers must read `m.id` for the model name to send.
...
- **`src/components/SettingsPanel.jsx`** — Floating settings *window* anchored top-right (absolute, z-30) of the chat pane. Header with minimize/close, glassy backdrop-blur, mango-gradient header tint. Body has four stacked sections: **Server** row (with `Server` icon + reconnect button), **System Prompt** preset list (radio-style rows including a "None" entry, `+ New` button, hover-revealed edit/delete per row, inline `PresetEditor` for create/edit with name input + content textarea), **MCP Tools** indicator (read-only — flat tool count + per-server `McpServerRow` with status pill `ready`/`error`, tool count, expand-to-see-tool-list; header pill says `sent to model`/`model has no tool_use`/`none`; empty state nudges to edit `mcp.json`), and **Model** card grid. Each `ModelCard` shows type badge (LLM/VLM/EMB), loaded-state pulse dot, quantization, publisher · arch, max-context (formatted K/M), `tools` capability chip. Embeddings are rendered as disabled. Includes a filter input over the grid. Selected card has a check badge + mango ring.
...
- **`vite-plugin-mcp.js`** — MCP bridge as a Vite middleware plugin. On dev-server start it reads `mcp.json` from the project root, spawns each declared server as a stdio child, runs the JSON-RPC handshake (`initialize` → `notifications/initialized` → `tools/list`), and exposes two endpoints:
  ...
  Per-request 15s timeout. Children are killed on `httpServer.close`, `exit`, `SIGINT`/`SIGTERM`/`SIGHUP`. Missing/empty `mcp.json` is a clean no-op (endpoints return empty list).
...
- **`src/api/mcp.js`** — Browser client for the bridge: `fetchMcpTools()` returns `{ tools, servers }` (or empty arrays on failure — degrades silently when the bridge is offline), `callMcpTool(name, args, signal)` returns `{ content, isError }`.
...
- **Adding a new MCP server**: drop it in `mcp-servers/` (any language — must speak MCP stdio), add an entry to `mcp.json` (`{command, args, env?}` shape, paths relative to repo root), restart `npm run dev`. Then the bridge will spawn it and auto-register its tools on boot.
